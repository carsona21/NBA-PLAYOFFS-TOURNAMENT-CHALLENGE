...
const elements = {
  publishedLabel: document.querySelector("#snapshot-published-label"),
...
async function fetchScoreboardEvents() {
  ...
  try {
    ...
    // Fall through to per-day fetches.
  }
...
function buildTeamAliasMap(game) {
  return Object.fromEntries(
    game.teams.map((team) => {
      const aliases = new Set([team.name].concat(team.aliases || []));

      if (team.name === "Los Angeles Lakers") aliases.add("Lakers");
      if (team.name === "Oklahoma City Thunder") aliases.add("Thunder");
      if (team.name === "San Antonio Spurs") aliases.add("Spurs");
      if (team.name === "Denver Nuggets") aliases.add("Nuggets");
      if (team.name === "Houston Rockets") aliases.add("Rockets");
      if (team.name === "Minnesota Timberwolves") {
        aliases.add("Timberwolves");
        aliases.add("Wolves");
      }
      if (team.name === "Phoenix Suns") aliases.add("Suns");
      if (team.name === "Portland Trail Blazers") {
        aliases.add("Trail Blazers");
        aliases.add("Blazers");
      }
      if (team.name === "Detroit Pistons") aliases.add("Pistons");
      if (team.name === "Boston Celtics") aliases.add("Celtics");
      if (team.name === "New York Knicks") aliases.add("Knicks");
      if (team.name === "Cleveland Cavaliers") {
        aliases.add("Cavaliers");
        aliases.add("Cavs");
      }
      if (team.name === "Toronto Raptors") aliases.add("Raptors");
      if (team.name === "Atlanta Hawks") aliases.add("Hawks");
      if (team.name === "Philadelphia 76ers") {
        aliases.add("76ers");
        aliases.add("Sixers");
      }
      if (team.name === "Orlando Magic") aliases.add("Magic");
      if (team.name === "Charlotte Hornets") aliases.add("Hornets");
      if (team.name === "Golden State Warriors") aliases.add("Warriors");
...
function findDraftTeamIdByEspnTeam(game, aliasMap, espnTeam) {
  ...
  const matchingTeam = game.teams.find((team) =>
...
function extractTeamStats(game, events) {
  ...
  const stats = Object.fromEntries(game.teams.map((team) => [team.id, { wins: 0, losses: 0 }]));

  events.forEach((event) => {
    const competition = event.competitions && event.competitions[0];
    const status = (competition && competition.status && competition.status.type) || (event.status && event.status.type);
    const competitors = (competition && competition.competitors) || [];

    if (!status || !status.completed || competitors.length !== 2) {
      return;
    }
    ...
    if (winnerTeamId) stats[winnerTeamId].wins += 1;
    if (loserTeamId) stats[loserTeamId].losses += 1;
  });
...
function getTeamDisplayName(game, teamId) {
  const draftTeam = getTeamById(game, teamId);
  return (draftTeam && draftTeam.name) || (TEAM_LIBRARY[teamId] && TEAM_LIBRARY[teamId].name) || "Unknown team";
}
...
async function renderSnapshotBoard() {
  const snapshot = window.NBA_PUBLISHED_SNAPSHOT;

  if (!snapshot || !snapshot.game) {
    elements.publishedLabel.textContent = "No published snapshot yet. Open the control room and download a published snapshot after the draft.";
    elements.sourceStatus.textContent = "Waiting for published draft data.";
    elements.standingsStrip.innerHTML = "";
    elements.squadGrid.innerHTML = "";
    return;
  }

  const game = buildDerivedGame(snapshot.game);

  elements.publishedLabel.textContent = `Published ${timestampToLabel(snapshot.publishedAt || snapshot.game.updatedAt)}`;
...
setInterval(renderSnapshotBoard, 30000);
renderSnapshotBoard();
